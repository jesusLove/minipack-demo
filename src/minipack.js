...
function createAsset(filename) {
  ...
  traverse(ast, {
    ...
    ImportDeclaration: ({node}) => {
      ...
      // 将依赖文件存入依赖中
      // 例如： import aaa from 'xxx.js'
      // node.source.value 就是 xxx.js
  ...
  // 不是所有的浏览器都支持 EcmaScript 和 JS 一些特性。为了在所有的浏览器中运行我们的 bundle 需要使用 Babel 转义
  // presets 是一个集合告诉 Babel 如何转义代码。这里使用 babel-preset-env 进行转义
  ...
  return {
    id, // 唯一 ID
    filename, // 文件名
    dependencies, // 依赖路径，就是 import 的内容
    code, // 转义后的代码
  };
}
...
function createGraph(entry) {
  ...
  // 使用队列来解析 asset 的依赖关系，开始只有 mainAsset 一个元素。
  ...
  // 使用 for...of 遍历队列，在迭代的过程中会向队列中推入新 asset
